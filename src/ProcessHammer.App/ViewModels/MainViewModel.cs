...
public sealed class MainViewModel : ViewModelBase
{
    ...
    {
        var dlg = new OpenFileDialog { Filter = "Process Hammer config (*.json)|*.json|All files (*.*)|*.*", Title = "Import config" };
    ...
    {
        var dlg = new SaveFileDialog { Filter = "Process Hammer config (*.json)|*.json", Title = "Export config", FileName = "process-hammer.json" };
        if (dlg.ShowDialog() != true) return;
    ...
    {
        var dlg = new SaveFileDialog { Filter = "Text report (*.txt)|*.txt", Title = "Export system report", FileName = "process-hammer-report.txt" };
        if (dlg.ShowDialog() != true) return;
        try
        {
            var text = await Task.Run(Hardware.ReportBuilder.Build);
