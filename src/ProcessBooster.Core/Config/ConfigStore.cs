...
namespace ProcessBooster.Core.Config;
...
public sealed class ConfigStore
{
    ...
    /// <summary>Default location: %LOCALAPPDATA%\ProcessBooster\config.json.</summary>
    ...
    public static AppConfig Deserialize(string json)
    {
        ...
        if (config.SchemaVersion > AppConfig.CurrentSchemaVersion)
            throw new InvalidDataException(
                $"Config schema v{config.SchemaVersion} is newer than supported v{AppConfig.CurrentSchemaVersion}. Update Process Booster.");
